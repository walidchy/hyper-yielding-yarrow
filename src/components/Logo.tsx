        ...
        <div className={cn('flex items-center gap-2', logoColor)}>
          ...
          fontFamily: "'Montserrat', 'Cairo', sans-serif"
            ...
            <div className={cn('flex', 'items-center', isRTL ? 'flex-row-reverse' : 'flex-row')}>
              ...
              {!collapsed ? <>
                  {letters.map((letter, index) => <div key={index} className={cn('border-4 rounded-md aspect-square flex items-center justify-center', size === 'sm' ? 'w-6 text-base' : size === 'md' ? 'w-10 text-xl' : 'w-12 text-2xl', logoColor, 'border-current', index > 0 ? 'mx-1' : '')}>
                      {letter}
                    </div>)}
                </> :
